...
public class DemoMessageStore {
	...
	public void push(ByteMessage msg, String topic) throws Exception {
		...
		synchronized (files) {
			if (!files.containsKey(topic)) {
				dataout = new DataOutputStream(new BufferedOutputStream(new FileOutputStream("data/" + topic, true),20480*1024));
	...
	ByteMessage pull(String topic) throws IOException {
		...
		if (!bufferinput.containsKey(toc)) {
			...
			DataInputStream datain = new DataInputStream(new BufferedInputStream(new FileInputStream("data/" + topic),20480*1024));
